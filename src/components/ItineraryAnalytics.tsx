...
const CATEGORY_COLORS: Record<string, { text: string; bg: string; border: string; bar: string }> = {
  cafe: { text: 'text-indigo-400', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20', bar: 'bg-indigo-500' },
  restaurant: { text: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/20', bar: 'bg-rose-500' },
  museum: { text: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20', bar: 'bg-purple-500' },
  temple: { text: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/20', bar: 'bg-amber-500' },
  park: { text: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', bar: 'bg-emerald-500' },
  'scenic-overlook': { text: 'text-sky-400', bg: 'bg-sky-500/10', border: 'border-sky-500/20', bar: 'bg-sky-500' },
  historic: { text: 'text-yellow-500', bg: 'bg-yellow-500/10', border: 'border-yellow-500/20', bar: 'bg-yellow-500' },
  custom: { text: 'text-slate-400', bg: 'bg-slate-500/10', border: 'border-slate-500/20', bar: 'bg-slate-400' },
};
...
export default function ItineraryAnalytics({ places }: ItineraryAnalyticsProps) {
  ...
  const primaryVibe = sortedCategories.length > 0 ? CATEGORY_LABEL[sortedCategories[0][0]] || sortedCategories[0][0] : 'Exploratory';
    ...
    <div className="p-3 border rounded-2xl glass-panel space-y-3.5 shadow-xl select-none">
      ...
      <div className="grid grid-cols-2 gap-2 text-left">
        ...
        <div className="p-2 bg-white/2 rounded-xl border border-white/5 space-y-0.5">
          <span className="text-[8px] uppercase tracking-wider text-slate-500 font-mono">Total Stops</span>
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-display font-medium text-white">{totalSpots}</span>
        ...
        <div className="p-2 bg-white/2 rounded-xl border border-white/5 space-y-0.5 truncate">
          <span className="text-[8px] uppercase tracking-wider text-slate-500 font-mono">Primary Atmosphere</span>
          <div className="flex items-center gap-1 mt-0.5 font-sans">
            <Sparkles className="w-3 h-3 text-indigo-400 shrink-0" />
            <span className="text-xs font-semibold text-slate-200 truncate">{primaryVibe}</span>
          </div>
        </div>
      </div>
              ...
              <div key={catKey} className="group flex flex-col space-y-1">
                <div className="flex items-center justify-between text-[10px] font-mono">
                  <span className="text-slate-300 flex items-center gap-1 shrink-0">
                    <span className="text-slate-400 w-4 font-normal text-right">{count}x</span>
                    <span className={`font-semibold ${style.text}`}>{CATEGORY_LABEL[catKey] || catKey}</span>
                  </span>
                  <span className="text-slate-500 text-[9px]">{Math.round(percentage)}%</span>
                </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-white/5 text-[9px] font-mono text-slate-400">
        ...
        <span className={geocodeRate === 100 ? 'text-emerald-400 font-bold' : 'text-indigo-300'}>
