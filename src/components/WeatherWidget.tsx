...
export default function WeatherWidget({
  ...
  const getWeatherDecoration = (code: number) => {
    if (code === 0) {
      return {
        condition: "Clear Sky",
        themeClass:
          "from-amber-500/20 via-orange-500/10 to-transparent border-amber-500/30 text-amber-200",
    ...
    if ([1, 2, 3].includes(code)) {
      return {
        ...
        themeClass:
          "from-blue-500/15 via-slate-500/10 to-transparent border-slate-500/30 text-slate-200",
    ...
    if ([51, 53, 55, 56, 57].includes(code)) {
      return {
        condition: "Drizzling",
        themeClass:
          "from-sky-500/20 via-blue-500/10 to-transparent border-sky-500/30 text-sky-200",
    ...
    if ([61, 63, 65, 66, 67, 80, 81, 82].includes(code)) {
      return {
        ...
        themeClass:
          "from-indigo-500/20 via-blue-600/10 to-transparent border-indigo-500/30 text-indigo-200",
    ...
    if ([71, 73, 75, 77, 85, 86].includes(code)) {
      return {
        condition: "Snowfall",
        themeClass:
          "from-cyan-300/15 via-slate-100/10 to-transparent border-cyan-400/30 text-cyan-100",
        icon: (
          <CloudSnow className="w-4 h-4 text-cyan-300 animate-bounce-slow" />
        ),
    ...
    if ([95, 96, 99].includes(code)) {
      return {
        ...
        themeClass:
          "from-purple-500/20 via-rose-500/10 to-transparent border-purple-500/40 text-purple-200",
        icon: (
    ...
    return {
      condition: "Overcast",
      themeClass:
        "from-slate-500/15 via-slate-800/10 to-transparent border-slate-700/55 text-slate-300",
  ...
  return (
    <div
      className={`p-0.5 rounded-2xl bg-gradient-to-br from-slate-900/90 to-slate-950/95 border border-white/5 shadow-2xl backdrop-blur-md select-none transition-all duration-500 w-full max-w-[280px] hover:shadow-indigo-500/5 hover:-translate-y-0.5 group`}
    >
      ...
      {weather && dec && (
        <div className="flex flex-col p-3 rounded-2.5xl space-y-2.5 relative overflow-hidden">
          {/* Accent glow halo */}
          <div
            className={`absolute -top-12 -right-12 h-20 w-20 rounded-full blur-3xl filter opacity-40 transition-colors duration-500 ${dec.glowColor}`}
          />
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <div
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border bg-gradient-to-r text-xs font-medium ${dec.themeClass}`}
            >
              {dec.icon}
              <span className="font-sans text-[10px] tracking-wide uppercase font-semibold">
                {dec.condition}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 pt-2 border-t border-slate-900 text-[10px] font-mono text-slate-400">
            ...
            <div className="flex items-center gap-1.5 p-1.5 bg-slate-950/45 rounded-lg border border-slate-900/30">
              ...
              <div className="flex flex-col truncate">
                ...
                <span className="text-slate-200 mt-0.5 leading-none font-bold">
                  {weather.humidity}%
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between text-[8px] font-mono text-slate-500 select-none pt-0.5">
            <span>
              Feels:{" "}
              {isFahrenheit
                ? `${weather.apparentTempF}°F`
                : `${weather.apparentTempC}°C`}
            </span>
