...
class ChatRequest(BaseModel):
    ...
    # FIX: was `json_schema Extra=` (SyntaxError) — corrected to `json_schema_extra`
...
class SpotSchema(BaseModel):
    ...
    # FIX: Removed overly-restrictive emoji regex — it rejected valid emojis like ⛩️, 🏰, 🍣.
    # The length validator below still ensures it's a single grapheme cluster.
    emoji: str = Field(..., description="Single descriptive emoji representing this place")
    address: str = Field(..., min_length=5, max_length=300, description="Geocodable address")
    # FIX: Aligned category values with frontend/Node.js backend categories.
    # Previous values (outlook, hotel, art, food, nature) did not match the frontend.
    category: str = Field(
        ...,
        pattern="^(cafe|restaurant|museum|temple|park|scenic-overlook|historic|custom)$",
        description="Place category matching frontend filter chips"
    )

    @field_validator('emoji')
    @classmethod
    def validate_emoji_length(cls, v):
        """Ensure emoji is a single visible character (grapheme cluster)."""
        import unicodedata
        # Strip variation selectors and count visible characters
...
class GeminiResponseSchema(BaseModel):
    ...
    # FIX: min_length/max_length are string constraints in Pydantic v2, not list constraints.
    # Use a field_validator to enforce list size limits instead.
    spots: List[SpotSchema] = Field(..., description="List of recommended POIs (1–10)")

    @field_validator('spots')
    @classmethod
    def validate_spots_count(cls, v):
        """Ensure between 1 and 10 spots are returned."""
        if len(v) < 1:
            raise ValueError("At least 1 spot is required")
        if len(v) > 10:
            raise ValueError("Cannot return more than 10 spots")
        return v
...
# FIX: Use the modern `lifespan` context manager only (removed deprecated
# @app.on_event("startup") which was redundant and conflicting).
@asynccontextmanager
    ...
    **Example Request:**
    ...
    try:
        ...
        system_prompt = (
            ...
            "Use only these category values: cafe, restaurant, museum, temple, park, scenic-overlook, historic, custom."
        )
